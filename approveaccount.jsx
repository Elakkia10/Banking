...
export const sendEmail = async (toEmail, templateId) => {
  try {
    const templateParams = {
      to_email: toEmail,
      firestore_api_link: 'https://firestore.googleapis.com/v1/projects/elakkia-bank/databases/(default)/documents/customerAccounts',
      // Add other template parameters here as needed
    };
 
    const serviceId = 'service_zc0q6d7'; // Replace with your EmailJS service ID
    const userId = 'ckcocI0f9uOA2wiUX'; // Replace with your EmailJS user ID
...
function ApproveAccountsPage() {
  ...
  const fetchPendingRequests = async () => {
    try {
      const response = await axios.get('https://firestore.googleapis.com/v1/projects/elakkia-bank/databases/(default)/documents/customerAccounts');
  ...
  const updateRequestStatus = async (requestId, newStatus, email, phoneNumber) => {
    try {
      ...
      await axios.patch(`https://firestore.googleapis.com/v1/projects/elakkia-bank/databases/(default)/documents/customerAccounts/${requestId}`, {
        fields: updatedFields
      });
 
      // Send email regardless of status
      const templateId = newStatus === 'approved' ? 'template_tizf22i' : 'template_sta4hdl'; // Use different template for rejection
      ...
      if (newStatus === 'approved') {
        ...
        await axios.post('https://firestore.googleapis.com/v1/projects/elakkia-bank/databases/(default)/documents/Account', {
          fields: {
            accountNumber: { integerValue: accountNumber },
